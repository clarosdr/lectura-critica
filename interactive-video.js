...
var interaction = {
	...
	updateEvaluationIcon: function () {
		if (interaction.ideviceID && interaction.ideviceID && interaction.evaluation && interaction.evaluationID.length > 0) {
			var node = $('#nodeTitle').text(),
				data = interaction.getDataStorage(interaction.evaluationID)
			var score = '',
				state = 0;
			if (!data) {
				interaction.showEvaluationIcon(state, score);
				return;
			}
			const findObject = data.activities.find(
				obj => obj.id == interaction.ideviceID && obj.node === node
			);
			if (findObject) {
			...
			interaction.showEvaluationIcon(state, score);
			var ancla = 'ac-' + interaction.ideviceID;
            $('#' + ancla).remove();
            $('.exe-interactive-video').eq(0).C
		}
	},
	showEvaluationIcon: function (state,score) {
		...
		$('#interactiveEvaluationIcon').find('span').eq(0).text(alt)
	},
	...
	saveEvaluation: function (score) {
		var score = ((interaction.score * 10) / interaction.numSlides).toFixed(2);
		if (interaction.ideviceID && interaction.evaluation && interaction.evaluationID.length > 0) {
			var name = $('.exe-interactive-video').parents('article').find('.iDeviceTitle').eq(0).text(),
				node = $('#nodeTitle').text(),
			...
			var data = interaction.getDataStorage(interaction.evaluationID);
			data = interaction.updateEvaluation(data, scorm);
			data = JSON.stringify(data, interaction.evaluationID);
			localStorage.setItem('dataEvaluation-' + interaction.evaluationID, data);
			interaction.showEvaluationIcon(scorm.state, scorm.score)

		}
	},
	getDataStorage: function (evaluationID) {
		var id = 'dataEvaluation-' + evaluationID,
			data = interaction.isJsonString(localStorage.getItem(id));
		return data;
	},
	...
	enable: function () {
		...
		this.idevicePath = this.isInExe ? "/scripts/idevices/interactive-video/export/" : "";
		...
		if (typeof (InteractiveVideo) != "undefined") {
			...
			interaction.evaluation = typeof InteractiveVideo.evaluation == "undefined" ? false : InteractiveVideo.evaluation;
			interaction.evaluationID = typeof InteractiveVideo.evaluationID == "undefined" ? '' : InteractiveVideo.evaluationID;
			interaction.ideviceID = typeof InteractiveVideo.ideviceID == "undefined" ? '' : InteractiveVideo.ideviceID;
		...
		if (interaction.ideviceID && interaction.evaluation && interaction.evaluationID.length > 0) {
			interaction.updateEvaluationIcon();
		}
 ...
 */
! function (e, t) {
	"function" == typeof define && define.amd ? define(["jquery"], t) : "object" == typeof exports ? module.exports = t(require("jquery")) : e.sortable = t(e.jQuery)
}(this, function (e) {
	"use strict";
	var t, a, r = e(),
		n = [],
